...
fn companion_rows(m: f64, q: f64) -> Vec<[f64; NUM_COEFFS]> {
    ...
    let high_formant = section_biquad(
        TYPE_PEAK,
        companion_geo(1_650.0, 2_550.0, m),
        2.8,
        5.0,
    );
    ...
    let low_companion = section_biquad(
        TYPE_PEAK,
        companion_geo(1_080.0, 1_220.0, m),
        5.0,
        3.0 * q,
    );
    ...
    let upper_companion = section_biquad(
        TYPE_PEAK,
        companion_geo(3_050.0, 3_350.0, m),
        5.0,
        3.0 * q,
    );
    vec![
...
fn source_gate(t: &dyn Target) -> SourceGateReport {
    ...
    for gesture in spec.gestures {
        ...
        for mi in 0..SOURCE_GATE_MQ {
            ...
            let cut_pass = cut.is_finite()
                && (gesture.cut_db.0..=gesture.cut_db.1).contains(&cut);
        ...
        for (si, row) in rows.iter().enumerate() {
                ...
                .all(|v| v.is_finite() && (-1.0e-9..=1.0 + 1.0e-9).contains(v))
            {
                return Err(format!(
                    ...
                    t.name(), CORNER_LABELS[ci], si, packed_values
                ));
            }
            ...
            if !radius.is_finite() || radius >= 1.0 {
                return Err(format!(
                    ...
                    t.name(), CORNER_LABELS[ci], si, radius
                ));
            }
...
fn authored_coefficient_loss(t: &dyn Target, pc: &PackedCorners) -> serde_json::Value {
    ...
    for (ci, &(m, q)) in CORNER_MQ.iter().enumerate() {
        ...
        let decoded = pc.interpolate_biquad(m as f32, q as f32);
...
fn authored_audition(
    ...
    let refs = [
        (
            "Talker",
            repo_root().join("plugin").join("presets").join("bodies").join("Talker.body240"),
        ),
        (
            "VOWL_iy_to_aa",
            repo_root().join("filters").join("bodies").join("VOWL_iy_to_aa.body240"),
        ),
    ];
...
fn cmd_talking_frame_stage_law() {
    ...
    .unwrap();
    println!("\nbody:   {}", root.join("talking_frame_stage_law.body240").display());
...
fn cmd_companion_mouth() {
    ...
    .unwrap();
    println!("\nbody:   {}", root.join("companion_mouth.body240").display());
    println!("audio:  {}", root.join("audio").display());
    println!("report: {}", root.join("report.json").display());
}
fn cmd_mouth() {
...
mod tests {
    ...
    fn authored_source_gate_rejects_branching_mouth_before_pack() {
        let t = BranchingMouth;
        let gate = source_gate(&t);
        assert!(!gate.passed, "the killed source must fail its companion-lobe gate");
        assert_eq!(
            gate.json["checks"]["intended_q_gestures"][0]["pass"],
            false
        );
    ...
    fn authored_route_directly_preserves_crossing_corner_lanes() {
        ...
        match route_target(&t, 0) {
            RouteResult::Candidate { pc, report } => {
                assert_eq!(
                    report["route"],
                    "AUTHORED_TRAJECTORY / direct corner pack"
                );
                assert_eq!(report["solver_invoked"], false);
    ...
    fn companion_mouth_source_gate_passes_with_positive_companion_lobes() {
        let t = CompanionMouth;
        let gate = source_gate(&t);
        assert!(gate.passed, "new authored candidate must pass its source gate: {}", gate.json);
    ...
    fn default_talking_frame_source_gate_passes_without_claiming_a_branch() {
        let t = TalkingFrameStageLaw;
        let gate = source_gate(&t);
        assert!(gate.passed, "default stage-law frame failed its source gate: {}", gate.json);
