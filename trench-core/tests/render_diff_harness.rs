...
fn render_rust_reference() -> Vec<f32> {
    ...
    // L/R cascades produce identical output for identical input — the left
    // channel is byte-equivalent to what the historical mono path produced.
    let input = load_or_generate_input();
