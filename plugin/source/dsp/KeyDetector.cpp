...
void KeyDetector::pushAudio (const juce::AudioBuffer<float>& buffer) noexcept
{
    ...
    while (sourceOffset < samples)
    {
        if (activeSlot < 0 && ! claimFreeSlot())
            return; // both complete slots are awaiting the message thread
                                 ...
                                 std::array<float, 12>& chroma) noexcept
{
    ...
    // Linear resampling is performed off the audio thread. At 44.1 kHz the
    // target samples land exactly every two source samples; other rates remain
