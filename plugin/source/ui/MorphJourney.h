...
// The response aperture is intentionally quiet in the product face. This is
// the one thing that earns that space back: a physical path between the two
// objects named by the body, with one live amber marker showing where the
// instrument is now. It is display-only; all sound and parameter ownership
// remains in the processor and trench-core.
...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        // The journey OWNS the screen — it is the display, not a strip above one.
        // Two named worlds at the top; the transformation rail centred in the
        // chamber as the hero; the amber marker is where the sound is between them.
        ...
        // A quiet calibration seam gives the rail its milled, instrument-panel
        // character without turning it into a generic progress bar.
        ...
        // The travelled portion is the only warm signal in the hero. At rest
        // it is a settled pilot light; while a take/keyframe moves it breathes
        // around the live position published by the processor.
        ...
        // End mouths: the path terminates in two small physical apertures,
        // keeping the visual language acoustic rather than software-linear.
...
private:
                            ...
                            juce::Rectangle<float> bounds, juce::Justification justification) const
    {
        ...
        // These labels sit on graphite glass, not on the beige face. Use the
        // quiet screen-ink token rather than the engraved plate ink, otherwise
        // the two physical endpoints disappear into the dark rail at runtime.
