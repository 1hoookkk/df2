...
// 5D — the extreme spatial toggle. One tap arms QSound's mind-bending orbit:
// the sound circles your head with exaggerated inter-aural cues. A dark hardware
// key like SEED/TAKE, but it LATCHES — lit with a teal ring while armed — since
// it is a mode, not a one-shot verb.
...
public:
    ...
    {
        // Match the reference: a small amber lamp + italic "5D" — engraved screen
        // furniture, NOT a badge. Lamp lit when armed; nothing else, no box.
        ...
        g.fillEllipse (lamp);
        g.setColour (juce::Colours::white.withAlpha (on ? 0.5f : 0.15f));   // glass catch
