...
{
    return juce::Font (juce::FontOptions ("DIN 1451 Mittelschrift", height,
                             ...
                             const juce::String& label, bool hover, bool down, const Theme& t)
{
    ...
    g.drawFittedText (label, area.translated (0, 1), juce::Justification::centred, 1);
...
inline void fillPhosphorGlass (juce::Graphics& g, juce::Rectangle<float> screen, const Theme& t)
{
    ...
    g.fillRect (screen);
    g.setColour (juce::Colour (0xfff0e3cc).withAlpha (0.045f));
    g.fillRect (screen);
    g.setColour (juce::Colour (0xff171411).withAlpha (0.08f));
    auto lower = screen;
                                     ...
                                     float catchAlpha = 0.45f)
{
    ...
    drawRun (juce::Colours::white.withAlpha (catchAlpha), 1.0f);
    drawRun (ink, 0.0f);
}
