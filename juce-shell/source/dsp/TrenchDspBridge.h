...
public:
    ...
    void process (juce::AudioBuffer<float>& buffer, const TrenchParams& params)
    {
        ...
        // The table is the always-on peak/envelope leveler; SLAM is applied after
        // the engine so it cannot change the body input or AGC pressure.
        // (Measured: at 1.0 the real P2K bodies sit at natural full-scale levels; >1
        // over-compresses them quieter and duller.)
        trench_engine_set_agc_drive (engine, musicalAgcDrive);

        const int n = buffer.getNumSamples();
        // SLAM is output-only in the plug-in: the selected body and AGC hear the
        // same clean input at every SLAM setting. The engine's own desk drive
        // stays OFF (slam param 0) so SLAM cannot move the filter excitation.
        const float slamNorm = juce::jlimit (0.0f, 1.0f, params.slamDrive);
        float intC = 0.0f;
        ...
        // Output-only SLAM: rounded pressure after the body. OUT CLIP now means
        // the pressure limiter had to round a sample.
        const float outC = trench::slamOutputPressureBlockStereo (buffer.getWritePointer (0),
                                                                 buffer.getWritePointer (1),
                                                                 n, slamNorm);
        slamIntClip.store (intC, std::memory_order_relaxed);
        slamOutClip.store (outC, std::memory_order_relaxed);
    }

    // SLAM metering (audio thread writes, UI reads): internal clip is always 0 in
    // the output-only path; output clip is post-filter pressure-limiter activity.
    float slamIntClipFrac() const noexcept { return slamIntClip.load (std::memory_order_relaxed); }
    float slamOutClipFrac() const noexcept { return slamOutClip.load (std::memory_order_relaxed); }
...
private:
    ...
    // SLAM meters (pre-filter internal clip is retired; output safety is post-SLAM).
    std::atomic<float> slamIntClip { 0.0f };
    std::atomic<float> slamOutClip { 0.0f };
