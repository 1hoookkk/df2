...
| TRENCH badge | — | click / shift-click | — | click cycles the face: CORAL BOUTIQUE (ships) ⇄ BORDEAUX LCD — signal tokens + both baked bitmaps swap live. Shift-click replays the onboarding tour (5 steps, demo body, lands on NO FILTER). The pick is a UI setting (`uiSkin` on the APVTS state tree), not a host parameter |
