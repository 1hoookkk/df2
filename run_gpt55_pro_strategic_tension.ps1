...
$promptPath = Join-Path $root 'gpt55-pro-strategic-tension-prompt.md'
$reportPath = Join-Path $root 'gpt55-pro-report-strategic-tension.md'
$reasoningPath = Join-Path $root 'gpt55-pro-reasoning-summary-strategic-tension.md'
$responsePath = Join-Path $root 'gpt55-pro-response-strategic-tension.json'
$responseIdPath = Join-Path $root 'gpt55-pro-response-id-strategic-tension.txt'
...
$evidenceExcerpts = @(
    ...
    @{ classification = 'OBSERVED-MORPH-DESIGNER'; path = 'dev\tmp\morph_designer_template_study\summary.md'; start = 1; count = 81 }
    @{ classification = 'OBSERVED-RE'; path = 'ref\ghidra_extracts\morphdesigner_types.md'; start = 8; count = 15 }
    @{ classification = 'OBSERVED-RE'; path = 'ref\ghidra_extracts\morphdesigner_types.md'; start = 96; count = 16 }
    @{ classification = 'OBSERVED-RE'; path = 'ref\ghidra_extracts\runtime_hacks.md'; start = 68; count = 18 }
)
...
foreach ($inputPath in @($promptPath) + @($evidenceExcerpts | ForEach-Object { $_.path })) {
...
Write-Host 'Submitting GPT-5.5 Pro DF2 productive-tension request...'
